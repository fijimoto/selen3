...
class Browser:
    ...
    def get_window_handles(self) -> list:
        """Алиас для get_all_handles"""
        return self.get_all_handles()
    ...
    def switch_to_new_window_handle(self, original_handle: str = None) -> str:
        ...
        Logger.info(
            f"{self}: switching to new window from handle '{original_handle}'")

        new_handle = self.get_new_window_handle(original_handle)

        self.switch_to_window_handle(new_handle)
        Logger.info(
            f"{self}: switched to new window with handle '{new_handle}'")
    ...
    def close(self) -> None:
        ...
        Logger.info(
            f"{self}: close window handle {self._driver.current_window_handle}")
        self._driver.close()

    def close_tab_by_index(self, index: int) -> None:
        """Закрытие страницы по индексу"""
        handles = self._driver.window_handles
        if index >= len(handles) or index < 0:
            raise IndexError(
    ...
    def close_tab_by_title(self, title):
        ...
        for handle in handles:
            if handle != self.main_handle:
                self.switch_to_window_handle(handle)
    ...
    def execute_script(self, script: str, *args) -> None:
        Logger.info(
            f"{self}: execute script = '{script}' with args = '{args}'")
        try:
    ...
    def switch_to_main_window(self) -> None:
        ...
        Logger.info(f"{self}: switch to main window")
        try:
    ...
    def switch_to_new_window(self) -> None:
        """Переключиться на новое окно (отличное от main_handle)"""
        Logger.info(f"{self}: switch to new window")
        for handle in self._driver.window_handles:
            if handle != self.main_handle:
                self._driver.switch_to.window(handle)
                return
        raise ValueError("New window not found")

    def wait_for_new_window(self, handles_before: list) -> None:
        """Ждать появления новой вкладки"""
        expected_count = len(handles_before) + 1
        Logger.info(f"{self}: wait for {expected_count} windows")
        self._wait.until(
            expected_conditions.number_of_windows_to_be(expected_count))
        ...
        while True:
            ...
            for handle in handles:
                ...
                if self._driver.title == title:
                    Logger.info(
                        f"{self}: new window handle = {self._driver.current_window_handle}")
                    return
    ...
    def switch_to_alert(self):
        ...
        return self.wait_alert_present()
